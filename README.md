...
> **להשלמה לפני ההגשה.** הצילומים נשמרים תחת `docs/screenshots/`. שורת הווידאו מחכה לקישור.

| # | מסך | קובץ | סטטוס |
|---|---|---|---|
| 1 | התחברות | `docs/screenshots/login.png` | להשלים |
| 2 | פיד האימונים, עם צ'יפים של סינון ומיון | `docs/screenshots/feed.png` | להשלים |
| 3 | העלאת אימון (הטופס עם התמונה) | `docs/screenshots/add_workout.png` | להשלים |
| 4 | פרטי אימון — כולל כפתור הלייק והתגובות | `docs/screenshots/details_light.png` | ✔ קיים |
| 5 | מועדפים | `docs/screenshots/favorites.png` | להשלים |
| 6 | פרופיל אישי | `docs/screenshots/profile.png` | להשלים |

בנוסף קיימים כבר: `details_dark.png` (אותו מסך במצב כהה), `details_dark_inactive.png` (לפני לייק
ולפני שמירה) ו‑`details_long_name.png` (שם משתמש ארוך שנחתך בשלוש נקודות).
...
| מסך המועדפים של משתמש אחר | הסטטיסטיקה מציגה `—` | `favorites` פרטי — וזו התנהגות מכוונת |
...
7. **בוליאני אחד לא הספיק לתאר את מצב החיבור.** באנר "אין חיבור לאינטרנט" הבזיק בכל הפעלה ובכל
   ...
   `NetworkGuard` עדיין שואל את `isOnline` ישירות ועדיין מסרב מיד.
...
| סוג | כמות | מה נבדק |
|---|---|---|
| יחידה (JVM) | 51 | `TimeFormatter`, `Validators`, `ErrorMapper`, חישובי הגודל של `ImageCompressor`, וכלל ההשהיה של באנר האופליין מול זמן וירטואלי |
...
| `DEMO_SCRIPT.md` | תסריט מדובר לווידאו ההדגמה, כולל מה על המסך בכל רגע |
| `ARCHITECTURE_QA.md` | 15 שאלות שמרצה עשוי לשאול, עם תשובות |
