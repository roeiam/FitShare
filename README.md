...
**פותח על ידי: רועי עמור (Roei Amor)**

קישור לווידאו ההדגמה נמצא בסעיף 2.
...
### רשימת היכולות — 17 יכולות ה‑MVP, כולן ממומשות

הרשימה כתובה כדי שאפשר יהיה לסמן עליה ✔ אחת‑אחת מול האפליקציה הרצה, ולכן כל שורה אומרת גם איפה
בקוד היא יושבת.

| # | היכולת | איפה בקוד |
|---|---|---|
| 1 | הרשמה עם אימייל, סיסמה ושם תצוגה | `ui/auth/RegisterFragment`, `AuthRepository.register` |
| 2 | התחברות לחשבון קיים | `ui/auth/LoginFragment`, `AuthRepository.login` |
| 3 | איפוס סיסמה במייל | `ui/auth/ForgotPasswordFragment` |
| 4 | שמירת החיבור בין הפעלות | `MainActivity.resolveStartDestination` |
| 5 | פיד חי (real‑time) של כל האימונים, החדשים למעלה | `WorkoutDataSource.observeFeed` על `snapshotListener` |
| 6 | סינון הפיד לפי קטגוריה | `FeedViewModel`, צ'יפים ב‑`fragment_feed.xml` |
| 7 | חיפוש בפיד לפי שם אימון או שם מתאמן | `FeedViewModel` — מסומן בקוד כ‑SPEC feature 7 |
| 8 | פרסום אימון עם תמונה מהמצלמה או מהגלריה, כולל דחיסה לפני העלאה | `ui/addworkout/`, `util/ImageCompressor` |
| 9 | מסך פרטי אימון מלא | `ui/details/WorkoutDetailsFragment` |
| 10 | לייק עם מונה חי | `ui/common/LikeButton`, `InteractionDataSource.toggleLike` |
| 11 | תגובות: הוספה, צפייה, ומחיקה של שלך | `ui/details/CommentAdapter`, `InteractionDataSource` |
| 12 | תמונת פרופיל למשתמש | `ui/profile/EditProfileFragment` — ראה מגבלה 5 בסעיף 9 |
| 13 | פרופיל אישי: שלוש סטטיסטיקות וגריד האימונים שפרסמת | `ui/profile/ProfileFragment` |
| 14 | עריכת פרופיל: שם, ביו ותמונה | `ui/profile/EditProfileFragment` |
| 15 | שמירה למועדפים, ורשימת מועדפים | `ui/favorites/`, `InteractionDataSource.toggleFavorite` |
| 16 | עריכה ומחיקה של אימון שאתה פרסמת | `AddWorkoutFragment` בתור עורך, `WorkoutRepository.deleteWorkout` |
| 17 | שיתוף אימון כטקסט | `WorkoutDetailsFragment` — מסומן בקוד כ‑SPEC feature 17 |

**מעבר ל‑MVP, ומומש גם הוא:**

| # | היכולת | איפה בקוד |
|---|---|---|
| 18 | באנר "אין חיבור לאינטרנט" | `util/NetworkMonitor`, `MainActivity.observeConnectivity` |
| 19 | מיון הפיד — חדשים או הכי אהובים | `data/model/FeedSort`, צ'יפים ב‑`fragment_feed.xml` |

**וגם:** צפייה בפרופיל של משתמש אחר (לקריאה בלבד), מצב כהה ובהיר עם מתג שנשמר בין הפעלות, וכל
האפליקציה בעברית עם RTL מלא.

> **על המספור.** מסמך האפיון עצמו היה קובץ עבודה ואינו חלק מהמסירה (ראה סעיף 11), ולכן המספור כאן
> הוא של README זה. הוא **לא** הומצא: שלוש הנקודות שהקוד עצמו מקבע — 7 (חיפוש), 12 (תמונת פרופיל)
> ו‑17 (שיתוף) — נשמרו במקומן, וכך גם 18 ו‑19 כתוספות שמעבר ל‑MVP, בדיוק כפי שהן מסומנות
> ב‑`FeedViewModel`, ב‑`WorkoutDetailsFragment`, ב‑`FeedSort` וב‑`PROGRESS.md`.
...
לאפליקציה **תשעה מסכים**, כולם פרגמנטים תחת NavHost אחד ב‑`nav_graph.xml`: התחברות, הרשמה,
שחזור סיסמה, פיד, מועדפים, הוספת אימון, פרופיל, פרטי אימון, ועריכת פרופיל. שבעת הצילומים שלמטה
מראים **חמישה** מהם — התחברות, פיד, הוספת אימון, מועדפים ופרופיל — ובנוסף את מסך פרטי האימון
בשתי ערכות הנושא, כלומר שישה מסכים בסך הכול. שלושת הנותרים — הרשמה, שחזור סיסמה ועריכת פרופיל —
אינם בצילומים.
...
**וגם `navigation-ui-ktx` אינה בפרויקט**, אף שהיא נפוצה בכל אפליקציית Navigation. היא הייתה כאן,
עד שהניווט התחתון עבר להיכתב ביד (ראה §3) ואיתו נעלם המשתמש היחיד שלה. `navOptions {}` מגיע
מ‑`navigation-common`, ש‑`navigation-fragment-ktx` גוררת בעצמה, ולכן אף שורה בקוד לא מייבאת
`androidx.navigation.ui`. תלות שאיש לא משתמש בה היא חוב, ולא ביטוח.
...
האפליקציה נפתחת על מסך ההתחברות. **פותחים חשבון חדש מתוך האפליקציה** — "אין לך חשבון? הרשמה" —
וממלאים שם תצוגה, אימייל וסיסמה.

**ההרשמה פתוחה ולא דורשת שום אישור.** אין רשימת הזמנות, אין אימות מייל שחוסם את הכניסה, ואין צעד
ידני בקונסולת Firebase. מרגע שההרשמה מצליחה המשתמש מחובר ומגיע ישר לפיד, והחיבור נשמר בין הפעלות
(יכולת 4) — הפעלה הבאה כבר נפתחת על הפיד ולא על ההתחברות.

מה שהטופס דורש, מתוך `util/Validators.kt`:

| שדה | הכלל |
|---|---|
| שם תצוגה | לפחות 2 תווים |
| אימייל | כתובת תקינה, ושאינה רשומה כבר |
| סיסמה | לפחות 6 תווים |
| אימות סיסמה | זהה לסיסמה |

הפיד שייפתח אינו ריק: בבסיס הנתונים יש 26 אימונים עם לייקים ותגובות, כך שאפשר לסנן, לחפש, למיין,
לעשות לייק, להגיב ולשמור למועדפים מיד — בלי לפרסם קודם אימון משלכם.

> **אין בקובץ הזה סיסמאות של חשבונות אמיתיים, ובכוונה.** הרפוזיטורי ציבורי, וחשבון אמיתי שסיסמתו
> כתובה ב‑README הוא חשבון פרוץ. חשבון חדש שנפתח מהאפליקציה נותן גישה מלאה לכל היכולות, כולל
> פרסום, עריכה ומחיקה של אימונים משלכם.
...
5. **SPEC §5 מזכיר תמונת פרופיל אופציונלית במסך ההרשמה; היא לא נבנתה.** משתמש חדש מגדיר תמונה
   דרך פרופיל ← עריכת פרופיל, וזה מכסה את יכולת **12** ברשימת היכולות שבסעיף 1 — מה שלא נבנה הוא
   הקיצור שבמסך ההרשמה, לא היכולת עצמה.
